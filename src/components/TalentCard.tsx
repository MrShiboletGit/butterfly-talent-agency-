    ...
    <Link to={`/talent/${talent.id}`} className="group block">
      ...
      >
        ...
        <div className="p-4">
          ...
          {/* Both rows render always. The per-platform breakdown used to appear only
              on hover, so touch visitors never saw it at all. */}
            ...
            <div className="flex items-center gap-3 text-xs text-gray-600">
              {talent.platformFollowers.youtube && (
                <div className="flex items-center gap-1">
                  ...
                  <span className="font-medium">{formatNumber(talent.platformFollowers.youtube)}</span>
                </div>
              )}
              {talent.platformFollowers.instagram && (
                <div className="flex items-center gap-1">
                  <Instagram size={14} className="text-pink-500" />
                  <span className="font-medium">{formatNumber(talent.platformFollowers.instagram)}</span>
                </div>
              )}
              {talent.platformFollowers.tiktok && (
                <div className="flex items-center gap-1">
                  <svg width="14" height="14" viewBox="0 0 24 24" fill="currentColor" className="text-black" aria-hidden="true">
                    <path d="M19.59 6.69a4.83 4.83 0 0 1-3.77-4.25V2h-3.45v13.67a2.89 2.89 0 0 1-5.2 1.74 2.89 2.89 0 0 1 2.31-4.64 2.93 2.93 0 0 1 .88.13V9.4a6.84 6.84 0 0 0-1-.05A6.33 6.33 0 0 0 5 20.1a6.34 6.34 0 0 0 10.86-4.43v-7a8.16 8.16 0 0 0 4.77 1.52v-3.4a4.85 4.85 0 0 1-1-.1z"/>
                  </svg>
                  <span className="font-medium">{formatNumber(talent.platformFollowers.tiktok)}</span>
                </div>
              )}
            </div>
          </div>
